...
int socket_buf_size = 10 * 1024 * 1024;
...
{
	static char addrstrcharbuf[max_addr_len + 10]; // leave 10 - 2 unused
	to_str(addrstrcharbuf);
	return addrstrcharbuf;
}
...
{ // works both for big and little endian
...
int new_connected_socket2(int &fd, address_t &addr, address_t *bind_addr, int isInClientMode)
{
	fd = socket(addr.get_type(), SOCK_DGRAM, IPPROTO_UDP);
	if (fd < 0)
	{
	...
	if (0 != connect(fd, (struct sockaddr *)&addr.inner, addr.get_len()))
	{
