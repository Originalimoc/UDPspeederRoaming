...
int fec_encode_manager_t::input(char *s, int len /*,int &is_first_packet*/)
{
	...
	if (fec_par.mode == 0)
		assert(counter < fec_par.queue_len); // counter will never equal fec_pending_num,if that happens fec should already been done.
	if (fec_par.mode == 1)
		assert(counter < fec_par.get_tail().x);
	...
	if (fec_par.mode == 0 && counter == fec_par.queue_len)
		about_to_fec = 1;

	if (fec_par.mode == 1 && counter == fec_par.get_tail().x)
		about_to_fec = 1;
		...
		if (fec_par.mode == 0)
		{
			...
			if (short_packet_optimize)
			{
				...
				for (int i = 1; i < tail_x; i++)
				{
					...
					if (shard_len > (u32_t)fec_par.mtu)
						continue;
